...
def get_qs(page):
    q_list = []
    result = requests.get("https://app.xmu.edu.my/AskA/?p={0}&CategoryID=0".format(page))
    ...
    for piece in tr_list:
        if piece.find_all(id="showDetail"):
            ...
            if q_body[-1] == '\n':
                q_body = q_body[:-2]
            q_list.append(
                Question(q_body, re.findall("\d{4}-\d{2}-\d{2}", piece.get_text())[0] + 'T'))
        else:
            ...
            for div in answer:
                ans_str += str(div)
            tmp_q = q_list.pop()
            tmp_q.body = re.sub(pattern='src="\/AskA\/', string=ans_str, repl='src="https://app.xmu.edu.my/AskA/',
